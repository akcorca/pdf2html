...
describe("clean.pdf paragraph merging", () => {
  ...
  it("removes residual soft-wrap hyphens in clean.pdf body prose", () => {
    expect(cleanHtml).toContain("summarizing the common steps of data standardization");
    expect(cleanHtml).toContain("process automatically with three steps");
    expect(cleanHtml).toContain("a Python Programmer , and a Code Executor");
    expect(cleanHtml).toContain("the historical conversational messages");
    expect(cleanHtml).toContain("across four attributes");
    expect(cleanHtml).toContain("can be directly generated by prompting");
    expect(cleanHtml).toContain("please not using any libraries");
    expect(cleanHtml).not.toContain("sum-marizing");
    expect(cleanHtml).not.toContain("au-tomatically");
    expect(cleanHtml).not.toContain("Pro-grammer");
    expect(cleanHtml).not.toContain("his-torical");
    expect(cleanHtml).not.toContain("at-tributes");
    expect(cleanHtml).not.toContain("di-rectly");
    expect(cleanHtml).not.toContain("li-braries");
  });
});
