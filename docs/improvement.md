...
3. 변환된 HTML 파일 중 하나의 문서를 집중하여 살펴보며 잘못 변환된 것으로 추정되는 부분을 찾는다. 제목이 아닌데 h1-h6 태그가 쓰였거나, 목록인데 li가 쓰이지 않았거나, 하나의 단락인데 여러 p 태그가 쓰였거나, 여러 단란인데 하나의 p 태그가 쓰였거나, 텍스트 추출이 잘못된 듯 텍스트의 내용이 이상하게 적혀 있거나(예를 들어 2단 레이아웃을 인식하지 못해서 텍스트가 섞이는 경우), 내부 링크가 걸려야 하는데 걸리지 않았거나 하는 등의 문제를 찾는다.
